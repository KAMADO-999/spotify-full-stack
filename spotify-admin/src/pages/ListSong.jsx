...
const ListSong = () => {
  ...
  const removeSong = async (id) => {
    try {
      // ✅ Correct way: pass id in URL params
      const response = await axios.delete(`${url}/api/song/remove/${id}`);
      if (response.data.success) {
        toast.success(response.data.message);
        await fetchSongs();
      ...
      <div>
        ...
        {data.map((item, index) => {
          return (
            <div
              key={index}
              className="grid grid-cols-[1fr_1fr_1fr] sm:grid-cols-[0.5fr_1fr_2fr_1fr_0.5fr] items-center justify-items-center gap-2.5 p-3 border border-gray-300 text-sm mr-5"
            >
              <img className="w-12" src={item.image} alt="" />
              <p>{item.name}</p>
              <p>{item.album}</p>
